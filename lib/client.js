...
/**
 * Simple chain queries using a single connection
 *
 * @param {mysql.Connection} connection
 * @param {Array.Array.(Query|Function)} cbqueries queries and associated callbacks
 * @param {Array.Object<T>|Object<T>} previousRes previous results (first is undefined)
 */
Client.prototype.chain = function(connection, cbQueries, previousRes) {
	var cbQuery = cbQueries.shift();
	var query = cbQuery[0];
	var callback = cbQuery[1];
	var self = this;
	connection.query(query.statement, query.args, function(err, res) {
		if (cbQueries.length > 0)
			callback(err, res, previousRes, function() {
				self.plain(connection, cbQueries, res);
			});
		else
			callback(err, res, previousRes);
	});
};
