...
Client.prototype.getConnection = function(callback) {
	this.pool.getConnection(function(err, connection) {
		if (err)
			throw err;
		// The callback should release the connection itself
		callback(connection);
	});
};
...
Client.prototype.multi = function(connection, queries, callback) {
	...
	statement += ';END;';
	connection.query(statement, args, function(err, res) {
		if (err)
			throw err;
		callback(res);
	});
...
Client.prototype._recTransaction = function(connection, queries, callback) {
	...
	connection.query(query.statement, query.args, function(err, res) {
		...
		if (query.affect && res.affectedRows === 0) {
			connection.query('ROLLBACK');
			throw new Error('No affected rows: ' + query.statement);
		}
		if (queries.length > 0)
...
Client.prototype.ptransaction = function(queries, callback) {
	...
	this.pool.getConnection(function(err, connection) {
		if (err)
			throw err;
		self.transaction(connection, queries, function() {
			connection.release();
			callback();
		});
	});
