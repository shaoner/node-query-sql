...
Query.prototype.whereIn = function(field, query) {
	...
	if (typeof query == 'string')
		this.statement += ' WHERE ' + field + ' IN (' + query + ')';
	else {
		this.values = this.values.concat(query.values);
		this.statement += ' WHERE ' + field + ' IN (' + query.statement + ')';
	}
	return this;
};
...
Query.prototype.where = function(condition) {
	...
	if (typeof condition == 'string')
		...
		this.statement += ' WHERE ' + this.hashToClause(condition, ' AND ');
	return this;
};

/**
 * Append a JOIN clause to this query
 *
 * @param {string} table
 * @param {Object<string, (string|number|boolean)>|string} condition
 * @return {Query}
...
Query.prototype.join = function(table, condition) {
	...
	if (typeof condition == 'string') {
		...
		this.statement += ' JOIN ?? ON ' + condition;
	} else {
		this.values.push(table);
		this.statement += ' JOIN ?? ON ' + this.hashToClause(condition, ' AND ');
	}
	return this;
};
...
Query.prototype.limit = function(offset, lines) {
	...
	if (arguments.length == 1)
		...
		this.statement += ' LIMIT ' + offset + ',' + lines;
	return this;
};
